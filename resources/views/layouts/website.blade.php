...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" href="{{ asset('website/css/summernote-bs4.css') }}">
      ...
      <div class="container">
        <div class="row mb-5">
          <div class="col-md-3">
            ...
            <p>{{ Illuminate\Support\Str::limit(strip_tags($setting->description,100)) }}</p>
            <p><a href="{{ route('website.about')}}">Read More</a></p>
          </div>
          ...
          <div class="col-md-4">
            ...
            <div>
              ...
              <p>
                @if($setting->facebook)<a href="{{$setting->facebook }}"target="_blank"><span class="icon-facebook pt-2 pr-2 pb-2 pl-0"></span></a>
                @endif
                @if($setting->twitter )<a href="{{$setting->twitter }}"target="_blank"><span class="icon-twitter p-2"></span></a>
                @endif
                @if($setting->instagram )<a href="{{$setting->instagram }}"target="_blank"><span class="icon-instagram p-2"></span></a>
                @endif
                @if($setting->reddit)<a href="{{$setting->reddit }}"target="_blank"><span class="icon-rss p-2"></span></a>@endif
                @if($setting->email)<a href="{{$setting->email }}" target="_blank"><span class="icon-envelope p-2"></span></a>
                @endif
              </p>
              <h3 class="footer-heading mb-4">Address</h3>
                @if($setting->address)<p>{{$setting->address}}</p>@endif
                @if($setting->phone)<h6>Phone:{{$setting->phone}}</h6>@endif
            </div>
          </div>
        </div>
        <div class="row">
          <div class="col-12 text-center">
            <p>
              {{$setting->copyright}} All rights reserved |Website Developed By <a href="aponashikmahmud007@gmail.com" target="_blank" >Ashik Mahmud</a>
